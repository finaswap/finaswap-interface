...
Currently there is two types of oracles which the sushi interface supports, Chainlink & FinaSwap TWAP.
...
Each price feed has a pair token0/token1, FNA/ETH or FNA/USD for example, decimals, and proxy. We use this information to construct mappings of the price feed to their respectful pair tokens.
...
If a newly added token is not in the @finaswap/default-token-list, it should be added.
